...
.controller('EntryPeriodCtrl', ['$scope', '$http', '$timeout', '$stateParams', 'ionicDatePicker', '$filter', 'ionicMaterialInk','ionicMaterialMotion','toaster','$ionicHistory','$state', '$cordovaLocalNotification', '$ionicLoading', '$cordovaSQLite','$ionicPopup', function($scope, $http, $timeout,$stateParams, ionicDatePicker, $filter,ionicMaterialInk,ionicMaterialMotion, toaster, $ionicHistory,$state,$cordovaLocalNotification, $ionicLoading, $cordovaSQLite, $ionicPopup) {
  ...
  $scope.formsubmit = function(isValid) {
      if (isValid) {
        ...
        $http.post($scope.url, {"user_id": $scope.loggedin_id,"startDate": $scope.formSubmission.startDate, "endDate": $scope.formSubmission.endDate, "description": $scope.formSubmission.description,  "email": $scope.loggedin_email, "uniquekey": $scope.uniquekey})
          .success(function(data, status) {
              ...
              if($scope.result.created == 1) {
                ...
                var query = "INSERT INTO periods(user_id, start, end, description, email, uniquekey, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)";
                $cordovaSQLite.execute(db, query, [$scope.loggedin_id, $scope.start, $scope.end, $scope.formSubmission.description, $scope.loggedin_email, $scope.uniquekey, $scope.today, $scope.today]);
                console.log("Done...");
    ...
    $scope.addNotification1 = function() {
      ...
      $cordovaLocalNotification.schedule({
          ...
          text: "Next probable period: "+$scope.nmpDate_ntfctn,
          title: "EasyPeriod",
          autoCancel: true,
          sound: true,
          firstAt: alarmTime,
    ...
    $scope.addNotification2 = function() {
      ...
      $cordovaLocalNotification.schedule({
          ...
          text: "Fertility phase: "+$scope.nfsdate_ntfctn +"-"+$scope.nfedate_ntfctn,
          title: "EasyPeriod",
          autoCancel: true,
          sound: true,
          firstAt: alarmTime,
...
.controller('WelcomeCtrl', function($scope,$http,$timeout,$rootScope,$ionicHistory,$state, ionicMaterialInk,ionicMaterialMotion,$filter,$cordovaSQLite,$cacheFactory,$ionicLoading, $cordovaLocalNotification) {
  ...
  $scope.doRefreshWP = function() {
    ...
    $ionicHistory.clearCache().then(function () {
      $timeout( function() {
        ...
        var randomHash = Math.floor((Math.random() * 987654321) + 10);
        var SyncURL = "http://orbachinujbuk.com/ionic_server/syncPeriods.php?email="+$scope.loggedin_email+"&hash="+randomHash;
        $http({
          cache: false,
          method: 'GET',
          url: ''+SyncURL,
        ...
        .success(function (data, status, headers, config) {
          for(var r=0;r<data.length;r++) {
            console.log(data[r]);
            $scope.tempData = data[r];
            var syncquery = "UPDATE periods set start=?, end=?, description=?, updated_at=? WHERE uniquekey=?";
            $cordovaSQLite.execute(db, syncquery, [data[r].start, data[r].end, data[r].description, data[r].updated_at, data[r].uniquekey]).then(function(res) {
              console.log(res.rowsAffected);
              if(res.rowsAffected == 0) {
                console.log($scope.tempData);
                var insertquery = "INSERT INTO periods(user_id, start, end, description, email, uniquekey, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)";
                $cordovaSQLite.execute(db, insertquery, [$scope.loggedin_id, $scope.tempData.start, $scope.tempData.end, $scope.tempData.description, $scope.loggedin_email, $scope.tempData.uniquekey, $scope.tempData.created_at, $scope.tempData.updated_at]);
                console.log("1 New Insertion");
              } else {
                console.log("1 Updation");
              }
            }, function (err) {
                console.error(err);
            });
           }
           $scope.syncedNotification();
        })
        .error(function (data, status, headers, config) {
          console.log(status);
        });
        // sync data...
        $ionicLoading.show({ template: '<div>Synced...</div>', noBackdrop: true, duration: 2000, delay: 500 });
      }, 1000);
    });

    $scope.syncedNotification = function () {
      $cordovaLocalNotification.schedule({
        id: 78987,
        title: 'EasyPeriod',
        text: 'Your app is synced with the server!'
